...
use client::game::{Board, Gamestate, Team};
...
impl MatchMaker {
    ...
    fn load_players(&mut self) {
        ...
        let jsons: Vec<_> = std::fs::read_dir(dir)
            .unwrap()
            .filter(|file| file.as_ref().unwrap().path().extension().unwrap() == "json")
            .collect();
        for file in jsons {
    ...
    fn run_match(&mut self) {
        ...
        command
            .args([
                ...
                "gym/scserver/test-client.jar",
                "--tests",
                &self.n.to_string(),
                "--name1",
                "player1",
                "--player1",
                player1_path.to_str().unwrap(),
                "--name2",
                "player2",
                "--player2",
                player2_path.to_str().unwrap(),
                "--port",
                &13060u16.to_string(),
            ])
            .stdout(io);
        ...
        let scores_start = output
            .rfind("=============== SCORES ================")
            .unwrap()
            + "=============== SCORES ================".len()
            + 1;
        let scores_end = output
            .rfind("=======================================")
            .unwrap()
            - 1;
...
impl Player {
    fn save_to_file(&self, path: &PathBuf) {
        let write_to = File::create(path.join(&self.bin_filename).with_extension("json"))
